...
<div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-12 pb-16 md:pb-0 border-t-2 border-stone-200">
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 border-t border-stone-100">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <div>
              ...
              <p className="text-xl text-stone-600 max-w-2xl">
                Check out some of the content I've written over the years.
              </p>
            </div>
            ...
            <button 
              ...
              className="group flex items-center gap-3 bg-stone-900 text-white px-8 py-4 hover:bg-orange-600 transition-colors duration-300"
            >
          ...
          <div 
            id="blog-container"
            className="flex overflow-x-auto gap-6 pb-8 hide-scrollbar snap-x snap-mandatory shadow-inner"
            style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
          >
            {blogs.map((blog, index) => (
              <div 
                key={index}
                className="min-w-[300px] md:min-w-[calc(25%-18px)] flex-shrink-0 snap-start group"
              >
                <a 
                  href={blog.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block h-full bg-white border-2 border-stone-200 p-8 hover:border-orange-600 transition-all duration-300 flex flex-col justify-between"
                >
                  <div>
                    <div className="text-sm text-stone-400 mb-4 font-medium uppercase tracking-wider">
                      {blog.date}
                    </div>
                    <h3 className="text-2xl font-bold mb-4 group-hover:text-orange-600 transition-colors">
                      {blog.title}
                    </h3>
                    <p className="text-stone-600 line-clamp-3 mb-8">
                      {blog.description}
                    </p>
                  </div>
